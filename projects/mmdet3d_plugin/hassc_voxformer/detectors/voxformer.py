...
class VoxFormer(MVXTwoStageDetector):
    ...
    def forward_pts_train(self,
        ...
        """Forward function'
        """
        outs = self.pts_bbox_head(img_feats, img_metas, target, scribble)
        # print(outs)
        losses = self.pts_bbox_head.training_step(outs, target, scribble, img_metas)
        return losses
    ...
    def forward_train(self,
        ...
        losses = dict()
        losses_pts = self.forward_pts_train(img_feats, img_metas, target, scribble)
    ...
    def forward_test(self,
        ...
        img_feats = self.extract_feat(img=img) 
        outs = self.pts_bbox_head(img_feats, img_metas, target, scribble)
        completion_results = self.pts_bbox_head.validation_step(outs, target, scribble, img_metas)
