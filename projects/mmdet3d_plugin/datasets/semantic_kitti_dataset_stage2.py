...
from projects.mmdet3d_plugin.hassc_voxformer.utils.ssc_metric import SSCMetrics
...
class SemanticKittiDatasetStage2(Dataset):
    def __init__(
        ...
        label_mapping = "./preprocess/label/semantic-kitti.yaml"
    ...
    def get_data_info(self, index):
        ...
        lga = self.get_lga_info(sequence, frame_id)
    ...
    def get_meta_info(self, scan, sequence, frame_id, proposal_path):
        ...
        target = numpy.where(numpy.logical_or(target == 255, target == 0), target, ones)
        target[target == 255] = 0
        gt_proposal_bin = target.astype(np.float32)
        #
        # gt_proposal_bin = proposal_bin

        meta_dict = dict(
            sequence_id=sequence,
            frame_id=frame_id,
            proposal=proposal_bin,
            gt_proposal=gt_proposal_bin,
    ...
    def get_lga_info(self, sequence, frame_id):
        lga_info = os.path.join('/home/ws/4090_backup/data/kitti_lga/sequences', sequence, 'lgas', frame_id + '.npy')
        lga = np.load(lga_info)
        lga = lga.reshape(256, 256, 32)
        return lga
    ...
    def get_gt_info(self, sequence, frame_id):
        ...
        if self.split == "train" or self.split == "val":
            target_path = os.path.join("/home/ws/data/semantic_kitti/dataset/sequences", sequence,
                                       "voxels", frame_id + ".label")
            target_annotated_data = np.fromfile(target_path, dtype=np.uint16).reshape((-1, 1))

            target_annotated_data = self.comletion_remap_lut[target_annotated_data]

            invalid_name = target_path.replace('.label', '.invalid')
            invalid_voxels = unpack(np.fromfile(invalid_name, dtype=np.uint8))

            target_annotated_data[np.isclose(invalid_voxels, 1)] = 255
            target = target_annotated_data.reshape((256, 256, 32))
